...
def process_data(data_str):
    games_json = {}
    games_data = data_str.strip().split("\n")

    for game in games_data:
        parts = game.split(": ")
        if len(parts) == 2:
            game_number_str = parts[0].split(" ")[1]
            if game_number_str.isdigit():
                ...
                for round_str in rounds:
                    color_counts = round_str.split(", ")
                    round_dict = {color.split(" ")[1]: int(color.split(" ")[0]) for color in color_counts}
                    rounds_list.append(round_dict)

                games_json[game_number] = rounds_list
...
Game 100: 5 green, 1 red; 4 blue, 8 red, 4 green; 1 blue, 3 red, 15 green; 1 blue, 15 green, 1 red; 2 red, 13 green
    """
    games_json = process_data(data_str)
